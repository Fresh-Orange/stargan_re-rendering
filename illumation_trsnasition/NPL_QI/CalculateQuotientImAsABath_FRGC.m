function CalculateQuotientImAsABath_FRGC(PersonList,OrderList,FileList,SavePath,ImWidth,ImHeight)
%%
...
for ithPerson=PersonList
    for ithImage=OrderList
        NumOfIm=NumOfIm+1;
        ImToCalculate=Read_Data_or_Image_Into_Matrix(ithPerson,ithImage,FileList);
...
%           QuotientIm=round(255*(mat2gray(QuotientIm)));
        ...
        if ithPerson<10
            sPerson=sprintf('00%d',ithPerson);
        elseif ithPerson<100
            sPerson=sprintf('0%d',ithPerson);
        else
            sPerson=sprintf('%d',ithPerson);
        end
        if ithImage<10
            sImage=sprintf('00%d',ithImage);
        elseif ithImage<100
