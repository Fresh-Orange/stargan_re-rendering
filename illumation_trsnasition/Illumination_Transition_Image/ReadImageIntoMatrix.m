...
clear temp;
%read sample image to decide the image size
samplepathname = strcat(FileList,'01','01.bmp');
[X,map] = imread(samplepathname);
[Width,Height] = size(X);
%initial M
...
for j=1:NumOfPersonTrained    
    ...
    for i=1:ImageAmountPerPerson
        ...
        if selectImageID<=9 
            imagename = sprintf('0%d.bmp',selectImageID);
        else
            imagename = sprintf('%d.bmp',selectImageID);
        end 
